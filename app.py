...
def cart():
    ...
    for i in product:
        ...
        for j in cart_product:
            original_price = float(j['productprice'])
            current_price = original_price
            
            # Use negotiated price if available and within allowed range
            if 'negotiated_price' in i:
                negotiated_price = float(i['negotiated_price'])
                min_allowed_price = original_price * 0.8
                if min_allowed_price <= negotiated_price <= original_price:
                    current_price = negotiated_price
            ...
            dummy.append(i['_id'])
            dummy.append(i.get('negotiated_price'))  # Add negotiated price if exists
            data.append(dummy)
    
    total_saved = original_total - total_price
...
def manageproducts():
    ...
    for product in products_list:
        ...
        item.append(product.get('negotiation_price', ''))  # Add negotiation price
        data.append(item)
...
def addproduct():
    ...
    negotiation_price=request.form.get("negotiation-price")
    ...
    if request.method == 'POST':
        ...
        for cart_item in cart_items:
            ...
            if product:
                ...
                if 'negotiated_price' in cart_item:
                    price = float(cart_item['negotiated_price'])
                else:
...
@app.route("/negotiate-price/<cart_id>", methods=['POST'])
def negotiate_price(cart_id):
    if 'username' not in session:
        return redirect("/")
    ...
    try:
        ...
        if cart_item:
            product = products.find_one({'_id': ObjectId(cart_item['product_id'])})
            original_price = float(product['productprice'])
            min_allowed_price = original_price * 0.8  # 20% discount maximum
            
            # Validate negotiated price is within allowed range
            if min_allowed_price <= negotiated_price <= original_price:
                # Calculate savings
                ...
                carts.update_one(
                    ...
                    {'$set': {'negotiated_price': str(negotiated_price)}}
                )
